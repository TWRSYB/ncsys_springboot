...
public class CornCobPurchaseServiceImpl extends ServiceImpl<CornCobPurchaseMapper, CornCobPurchaseDo> implements CornCobPurchaseService {
    ...
    public ResVo<Object> saveTrade(MixedCornCobPurchaseDo mixedCornCobPurchaseDo) {
        ...
        if (ObjectUtils.isEmpty(mixedCornCobPurchaseDo.getSerno())) {
            ...
            mixedCornCobPurchaseDo.setCreateUser(sessionUserDo.getLoginCode());
        }
        mixedCornCobPurchaseDo.setUpdateUser(sessionUserDo.getLoginCode());
    ...
    private boolean validateMixedTrade(MixedCornCobPurchaseDo mixedCornCobPurchaseDo, String step) {
        ...
        if (!"save".equals(step)) {
            ...
            if ("settle".equals(step)) {
                // 结算金额必须等于总金额+补价
                BigDecimal clearingAmount = mixedCornCobPurchaseDo.getClearingAmount();
                BigDecimal premium = mixedCornCobPurchaseDo.getPremium();
                if (clearingAmount.compareTo(totalPrice.add(premium)) != 0) {
                    throw new BusinessException("结算金额必须等于总金额+补价");
                }
                // 结算日期不能为空
                String actualClearingDate = mixedCornCobPurchaseDo.getActualClearingDate();
